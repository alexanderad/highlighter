...
function pronounce(text) {
  ...
  if (dutchVoices.length > 0) {
    const randomVoice =
      dutchVoices[Math.floor(Math.random() * dutchVoices.length)]
    u.voice = randomVoice
  }
...
function drawWord(data) {
  ...
  document
    .getElementById("id-word")
    .addEventListener("click", () => pronounce(data["word"]))
  document.getElementById("id-example").innerHTML = data["example"].replace(
    data["word"],
    `<span class="highlight">${data["word"]}</span>`
  )
  document
    .getElementById("id-example")
    .addEventListener("click", () => pronounce(data["example"]), false)
...
async function cacheWords() {
  const promised = Array(cacheSize)
    .fill()
    .map(() => fetchWord())
  ...
  await chrome.storage.sync.set({
    words: storage.words.concat(downloadedWords),
  })
}
